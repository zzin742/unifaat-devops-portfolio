...
output "ssh_command" {
  description = "Comando pronto para conectar na instancia (chave baixada do painel do lab)."
  value       = "ssh -i ${var.ssh_key_path} ec2-user@${aws_instance.api.public_ip}"
}

output "instance_profile_usado" {
  description = "Instance profile do lab reusado - evidencia de que nada de IAM foi criado."
